...
- UI に関わる変更をするときは、**必ず [docs/design/style-guide.md](docs/design/style-guide.md) に従う**。
  色・タイポグラフィ・余白・角丸・部品（ボタン / リスト / カード / モーダル / タグ / アイコン）・
  モーション・状態表現のルールをここにまとめている。アートスタイルの一貫性を最優先する。
- デザイントークンの実体は [src/renderer/src/styles.css](src/renderer/src/styles.css) の `:root` / `[data-theme='dark']`。
  新しい色や寸法をハードコードせず、既存トークンから引く。
- スタイルガイドで表現できない見た目が必要になった場合は、実装前に確認する。
  規則を追加・変更したときは、スタイルガイドも同時に更新する（更新日時も更新）。
