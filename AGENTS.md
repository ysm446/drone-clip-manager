...
## UI スタイルガイド（UI 変更時は必須）

UI に関わる変更（画面・レイアウト・配色・タイポグラフィ・余白・新規コンポーネント・CSS 追加）を行うときは、
**作業前に [docs/design/style-guide.md](docs/design/style-guide.md) を読み、その規則に従う。**

- 色・サイズ・余白・角丸・トランジションは、`src/renderer/src/styles.css` のデザイントークンと
  スタイルガイドの値から選ぶ。生の色コードやトークン外の数値を新しく増やさない。
- 新しい見た目を足す前に、スタイルガイドの「部品ルール」で表現できないかを先に確認する。
- ホバー / 選択 / 無効 / 危険 の状態表現は、既存の共通ルールに合わせる。
- スタイルガイドに反する実装が必要な場合は、実装前にユーザーに確認する。
- 新しい規則を追加・変更した場合は、実装と同時に `docs/design/style-guide.md` を更新し、更新日時も書き換える。
- UI 変更後は、スタイルガイド末尾の「UI 変更時のチェックリスト」で確認する。
...
- `docs/design/` 配下は UI / 見た目に関する設計資料を置く場所として使う（`style-guide.md` が UI の正本）。
