...
// BGM のビート解析の純計算部（Phase 2.6c）。
//
// **このファイルは electron / fs に依存しない**こと。解析は 3 分の曲で 0.4〜1.6 秒かかる
// 同期計算なので、worker_threads（beatsWorker.ts）で実行してメインプロセスを
// ブロックしないようにしている（デコード・キャッシュは beats.ts 側）。
//
...
export const SR = 11025 // 解析用サンプルレート
...
/** 解析結果のうち、パス・所要時間を除いた純計算部分（worker から返す形） */
export type CoreBeatAnalysis = Omit<BeatAnalysis, 'relPath' | 'analyzedMs'>
...
/** デコード済み PCM からビート解析を行う（同期・重い。worker で呼ぶこと）。 */
export function analyzePcm(pcm: Float32Array): CoreBeatAnalysis {
