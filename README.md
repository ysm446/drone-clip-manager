...
DJI 等の HEVC 10bit 素材は、まず **原本のまま直接再生** を試みます（Chromium の platform HEVC デコーダ経由）。
Windows では別途 Microsoft Store の「**HEVC ビデオ拡張機能**」が必要な場合があります。

原本を再生できない環境では、プレイヤー上のボタンから **一時的に H.264 プロキシへ変換して再生** できます
（`.dcm/` には保存せず、OS 一時フォルダに置いてアプリ終了時に削除。書き出しは常に元素材を使うため無劣化）。
