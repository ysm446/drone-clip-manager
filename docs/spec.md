...
**プロジェクト名**: `drone-clip-manager`

> ⚠ **このドキュメントは Phase 2 時点の仕様で止まっている**（2026-08-16 時点）。
> Phase 2.5 以降で実装したもの——**クリップ一覧ビュー**（2.5）、**シーケンス**（2.6: ノードグラフ /
> 無劣化連結書き出し / 分割書き出し）、**BGM 合成**（2.6b）、**音楽タイムライン**（2.6c: 曲の波形の上に
> クリップを秒で自由配置し、隙間を黒で埋めて書き出す）——は**ここに反映されていない**。
> データモデルも `sequences` / `sequence_nodes` / `sequence_edges` / `sequence_bgm` が未記載。
> **これらの正本は [plan/plan.md](plan/plan.md)**（設計方針・検証結果）と
> [plan/progress.md](plan/progress.md)（実装状況）。UI は [design/style-guide.md](design/style-guide.md)。
...
- 書き出しキューと進捗表示
- **BGM 再生**: ユーザー指定の BGM フォルダ（mp3 等）を読み込み、編集中に再生できる簡易プレイヤー（§13）
...
- ルートフォルダは複数持てる想定だが、**メタデータは各ルートごとに、そのルート直下の `.dcm/` に閉じる**（ルート間で DB を共有しない）。
- `videos.path` はルートからの相対パスで持ち、ルートフォルダを移動しても参照が壊れないようにする（絶対パスが必要な処理では実行時にルートと結合して解決する）。
- `videos.thumb_path` も同様に `.dcm/` 起点の相対パスで保持する。
...
1. **取り込み**: **ルートフォルダを指定**（ユーザーがダイアログで選択。固定しない） → ルート直下に `.dcm/` を用意 → ルート配下を再帰走査で動画検出 → ffprobe でメタ抽出 → キーフレーム抽出 → サムネイル生成 → `.dcm/library.db` に登録
...
1. **プレビュー再生の HEVC 10bit / D-Log 問題** → **対応済み（mpv 埋め込みを主、<video> をフォールバック）**
   実機で `<video>`(Chromium) は HEVC 10bit を再生できないことを確認（プロキシ H.264 なら再生可＝純粋にデコーダの問題）。
   採用方針は **mpv(libmpv) の埋め込み再生**。原本を HW デコードで無劣化・キャッシュ不要・即スクラブ再生する。
   - 構成: メインプロセスが mpv を子プロセスで起動し、`--wid` で「動画領域だけを覆う子ウィンドウ」に埋め込む。制御は JSON IPC(名前付きパイプ)。
     - レンダラは動画領域の矩形（`getBoundingClientRect`）を送り、メインが子ウィンドウを画面座標に配置（リサイズ/移動に追従、モーダル表示中や未選択時は隠す）。
     - 再生/一時停止/シーク/音量は IPC、再生位置・長さは mpv の `observe_property`(time-pos/duration/pause/eof) を購読してタイムラインへ反映。
     - タイムラインは動画の「下」にある（動画に HTML を重ねない）ので、ネイティブ描画のネイティブウィンドウ方式と相性が良い。
   - mpv バイナリ: 開発時は winget の `shinchiro.mpv`（`C:\Program Files\MPV Player\mpv.exe`）を検出。`DCM_MPV_PATH` でも指定可。**配布時はアプリに同梱**する想定（未実装）。
   - フォールバック: mpv が見つからない/起動失敗の場合は従来の `<video>` 経路 + 一時プロキシ（H.264 8bit 720p、OS 一時フォルダ・終了時削除・永続キャッシュなし）に自動で退避。
   - 書き出しは常に元素材を使う（再生系とは独立）。
...
編集中にドローン映像へ合わせる BGM を試聴できるよう、簡易 BGM プレイヤーを持つ。

- **BGM フォルダはルートフォルダとは独立に、ユーザーが指定** する（mp3 等を置いたフォルダ）。指定値はアプリ設定に永続化する。
- 対応拡張子は音声一般（`.mp3` / `.m4a` / `.aac` / `.wav` / `.flac` / `.ogg` / `.opus`）。フォルダ配下を再帰走査して一覧化する。
- 再生は Renderer の `<audio>` で行い、動画とは独立に鳴らせる（映像を見ながら BGM を試聴）。
- 操作: トラック選択 / 再生・一時停止 / 前後送り / 音量 / リスト全体ループ。
- 配信は動画と同じ独自プロトコル（`dcm-media://bgm/<rel>`）で Range 対応。
- ~~スコープ外（今回やらない）: BGM を書き出しクリップにミックスすること~~
  → **実装済み**（Phase 2.6b / 2026-08-14）。連結書き出しに BGM を載せられる。
  **映像は stream copy のまま、音声だけ AAC** にエンコードするので「映像は無劣化」は保っている。
  仕様は [plan/plan.md](plan/plan.md) の「Phase 2.6b」を参照。

> 内部識別子メモ: メタデータディレクトリは各ルート直下の `.dcm/`、メディア配信の独自スキームは `dcm-media`（`root` / `bgm` を host で区別）。
