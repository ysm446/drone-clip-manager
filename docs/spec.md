...
1. **プレビュー再生の HEVC 10bit / D-Log 問題** → **対応済み（原本直接再生を優先。プロキシは容量を食わない一時フォールバック）**
   方針: **まず原本をそのまま再生**する。Chromium の platform HEVC デコーダを有効化（`app.commandLine.appendSwitch('enable-features','PlatformHEVCDecoderSupport')`）し、
   Windows の「HEVC ビデオ拡張機能」+ GPU があれば HEVC(10bit 含む) を原本のまま再生できる。
   - 再生できない機種向けフォールバック: **一時プロキシ**（H.264 8bit / 720p / 30fps）を生成して再生に切り替える。
     - **永続キャッシュを作らない**（容量対策）。OS 一時フォルダ（`app.getPath('temp')/drone-clip-manager/proxies`）に置き、**起動時・終了時に削除**する。
     - 自動では作らず、**再生失敗を検出したときにボタンで手動生成**（`<video>` の error、または再生後に時間が進まないヒューリスティックで検出）。
     - エンコーダは NVENC（cuda デコード）→ libx264（`-hwaccel auto` でデコードだけ HW 支援）の順にフォールバック。
   - 書き出しは常に元素材を使う（プロキシは再生専用）。
   - 未確定: それでも重い/不可の環境向けに libmpv 埋め込み（原本を HW デコードで軽くスクラブ）を将来検討。
