...
1. **プレビュー再生の HEVC 10bit / D-Log 問題** → **対応済み（選択肢 b を採用）**
   実機確認で、Electron/Chromium は DJI の HEVC 10bit（yuv420p10le）を直接再生できず、再生ボタンを押しても進まないことを確認。
   → **プレビュー用プロキシ**（H.264 8bit / 720p / 30fps、`.dcm/proxies/` にキャッシュ）を生成して再生に使い、書き出しは元素材を使う方式を実装（`ProxyService`）。
   - コーデック判定: `h264` かつ 8bit の素材はそのまま再生。それ以外（HEVC / 10bit / av1 等）はプロキシで再生。
   - 生成は「選択した動画を初めて開いたとき」にオンデマンド。2回目以降はキャッシュから即再生。
   - エンコーダは NVENC（cuda デコード）→ libx264（`-hwaccel auto` でデコードだけ HW 支援）の順にフォールバック。
   - 未確定: 取り込み時に全件プリ生成するか（現状はオンデマンド）。プロキシ解像度/fps の既定値。
