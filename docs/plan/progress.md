...
### シーケンス編集の残作業
- [ ] **BPM / 1 拍目の手動指定**（2026-08-15 起票 / 拍子より優先）: 検出したテンポが外れる曲がある
  （`Green From Above 2.wav`。ユーザーの耳では 3 拍子だが、検出は別の刻みに乗っている）。
  **拍の位置がずれていると拍子だけ直しても意味がない**ので、こちらが前提条件になる。
  - `sequence_bgm` に `bpm` / `first_beat_sec` を追加（NULL で自動）。拍子と同じ扱い。
  - 手動値があるときは**検出したビート列を捨てて等間隔グリッドを生成**する
    （`beats[i] = firstBeatSec + i × 60 / bpm`）。検出が外れるのはテンポが素直な曲が多く、
    複雑なリズムでは DP 追従が効くので、実用上は噛み合う。
  - 導線: BGM パネルのタップテンポ / 半・倍テンポで正しい BPM を割り出し、音楽ビューへ入力する。
    1 拍目はタイムライン上で「ここが 1 拍目」とクリック指定するのが直感的。
  - **教訓**: 拍子の自動判定（位相別コントラスト）は、拍の位置が正しいことを前提にしている。
    拍が外れていれば判定も一緒に外れるので、計測値を根拠に「3 拍子ではない」と結論しないこと。
- [x] **拍子（beatsPerBar）を可変にする**（2026-08-15）: `beats.ts` の 4 固定をやめ、拍子候補
  （2/3/4/6）ごとに小節頭の位相とコントラストを求めて全候補を返すようにした（`meters`）。
  自動判定は「3 が 4 を 15% 以上上回るときだけ 3」という控えめな基準で、迷ったら 4 に倒す。
  `sequence_bgm.beats_per_bar`（NULL で自動）に手動指定を保存し、**手動を常に優先**する。
  ヘッダの拍子セレクタで切り替えると、小節線・小節番号・「全 N 小節」・吸着単位・尺表示・
  レイアウトの起点がすべて連動する。解析キャッシュはキーに `v2` を入れて作り直させる。
  - **注意**: 拍が正しく検出できている曲でのみ有効。テンポが外れている曲は上記の手動指定が要る。
- [ ] **再生ボタンを上部プレイヤーへ一本化する**（2026-08-15 起票）: 音楽ビューのツールバーと
  ...
  - 残: ①音楽ビューが「拍に合わせた in/out + BGM」を**常時 App へ装填**する
  ...
  - **注意**: ①は再生中にキューが差し替わると位置が飛ぶ。装填条件（再生中は行わない /
    ...
    （停止後に鳴り出す / 先頭に戻る）。
- [ ] **タイムラインのクリック吸着が効いて見えない**（2026-08-15 起票 / 未解決）:
  吸着単位（既定 1 小節）へ丸める実装を入れたが、実機では小節線に乗って見えないとの報告。
  **切り分けが未了**。再生ヘッドは「クリック位置」ではなく **`dcm:seq-progress` の進捗率から
  求めた実際の再生位置**を描いているため、①吸着の計算が誤っている ②吸着は正しいが表示側が
  ズレて見える（再生中はすぐ先へ進む / 動画シークのフレーム丸め / `seqLocate` が
  クリップ終端 0.2 秒手前でクランプ）の 2 通りがありうる。
  - **進め方**: まずクリック位置と吸着後の位置を数値で出して切り分ける
    （例: ステータスバーへ「クリック 12.34s → 吸着 11.25s（5 小節目）」）。
    推測で修正を重ねない。
- [x] **ノードのコピー / 貼り付け**（2026-08-15）: 複数選択 → `Ctrl+C` → 別シーケンスで `Ctrl+V`。
  クリップボードは segment id の配列で、**順路順に並べ直してから**控える（矩形選択は飛び飛びに
  選べるため）。**`units` は持っていかない**（曲が違えば拍の長さも違うので、貼り付け先では
  自動から始める。新規ノードは既定値で作られるため追加処理は不要）。
...
- [x] **同一シーケンス内でのクリップ重複に対応**（2026-08-15）:
  ...
  - 同じクリップを 2 回置いた場合、**尺（`units`）と使用範囲（`src_offset`）はノードごとに
    別々に持てる**（元から可能だった）。同じ画を「前半 2 小節」「後半 4 小節」で使い分けられる。
  - `npm run build` / `typecheck` 成功（UI の実操作は未確認）。
