...
- [ ] **音楽同期タイムライン 段階 3 以降（Phase 2.6c）**: DnD 配置 / 並べ替え・尺の単位変更・
  `src_offset` スライド・曲差し替えと自動縮小 → 書き出し対応。`sequence_nodes` に
  `units` / `unit_kind` / `src_offset` / `auto_shrunk` を追加する。方針は [plan.md](plan.md)。
