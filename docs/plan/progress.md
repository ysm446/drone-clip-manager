...
- [ ] **音楽同期タイムライン 段階 4（Phase 2.6c）**: 書き出し対応。`units` から求めた out を
  使って連結書き出しし、曲の開始オフセットを反映する。方針は [plan.md](plan.md)。
- [ ] 線形シーケンス（ドラッグ並べ替え UI）: ノードグラフで代替中。→ Phase 2.6c の音楽タイムライン
  （タイムライン上での DnD 配置・並べ替え）で解消する想定。
