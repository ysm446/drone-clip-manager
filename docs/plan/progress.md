...
### ★ Phase 2.5 — クリップ一覧ビュー（区間を独立クリップとして横断一覧）※次の主要 UI
- [ ] ライブラリ ⟷ クリップの表示切替、クリップカード（in サムネイル / 元動画 / ラベル / in–out・長さ / バッジ）
- [ ] クリップから元動画を開いて in へシーク（範囲ループ再生も検討）、ラベル編集 / 削除
- [ ] 複数選択 → まとめて書き出し（動画横断）。Export モーダルをクリップ選択ベースに拡張
- [ ] 全区間取得 IPC（`segments:listAll` 結合）、in 点サムネイルを `.dcm/thumbnails/` にキャッシュ
- 詳細は [plan.md](plan.md) の「Phase 2.5」を参照

### Phase 2.6 — クリップのシーケンス（つないで連続再生 → 連結書き出し）
- [ ] 線形シーケンス（まず本命）: クリップを並べ替え → mpv プレイリストで連続再生 → ffmpeg concat で無劣化連結書き出し
- [ ] ノードグラフ（発展）: クリップをノードとして線でつなぎ順路を組んで再生（分岐は後回し）
