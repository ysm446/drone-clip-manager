...
- [ ] **同じクリップを 1 シーケンスに 2 回置けるようになった件の追従**（2026-08-15 起票）:
  コピー / 貼り付けの追加で、同一シーケンス内に同じ `segment.id` が複数入りうるようになった。
  **分割書き出し（`ExportModal`）は行を `segment.id` で管理している**ため、重複があると
  正しく動かない可能性がある（従来は「2 回置く運用は無い」前提だった）。
  別シーケンスへのコピーが主用途なら影響しないが、同一シーケンス内で重複させる使い方を
  するなら `ExportModal` の行キーをノード id 基準へ変える必要がある。
