...
- [x] **音楽同期タイムライン 段階 1: ビート検出 + BGM プレイヤー表示**（Phase 2.6c / 2026-08-14）:
  ...
  - `npm run build` / `typecheck` 成功。実装をプロトタイプと突き合わせて同値を確認
    （UI の実操作は未確認）。
- [ ] **音楽同期タイムライン 段階 2 以降（Phase 2.6c）**: 波形 + 小節グリッドの線形タイムライン表示 →
  DnD 配置 / 並べ替え・尺の単位変更・`src_offset` スライド・曲差し替えと自動縮小 → 書き出し対応。
  クリップの尺を秒ではなく拍 / 小節で持ち、曲を差し替えると全クリップが自動で並び直る。
  前詰め固定・縮める方向のみ自動・元 segment は不変。方針は [plan.md](plan.md) の「Phase 2.6c」。
