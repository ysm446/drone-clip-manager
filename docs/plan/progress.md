...
- [ ] **同一シーケンス内でクリップが重複したときの分割書き出し**（2026-08-15 起票 / 対応保留）:
  コピー / 貼り付けの追加で、同一シーケンス内に同じ `segment.id` が複数入りうるようになった。
  **分割書き出し（`ExportModal`）は行を `segment.id` で管理している**ため、重複があると
  **行が 1 つにまとまり、出力ファイルが 1 本足りなくなる**（エラーにならず連番が欠けるだけなので
  気づきにくい）。
  - **判断（2026-08-15）**: 同一シーケンスへのコピーは想定しないため**今は直さない**。
    ただし重複が起きること自体は許容する方針なので、実際に困ったら
    `ExportModal` の行キーを `segment.id` からノード id 基準へ変える。
