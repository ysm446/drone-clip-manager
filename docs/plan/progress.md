...
- フェーズ: **Phase 2.6 着手（シーケンス / ノードグラフ編集・一本道・再生まで実装）**。Phase 2.5（クリップ一覧ビュー）完了。
  Phase 2（MVP: 管理・区間マーク・ロスレス書き出し）到達済み。次は順路の無劣化連結書き出し。
...
- [ ] 範囲ループ再生（クリップの in–out をループ再生する再生モード）※ plan の「検討」項目
...
- [ ] **連結書き出し（無劣化 concat）**: 順路を ffmpeg concat demuxer で stream copy 連結して 1 本に書き出す（次の反復）。
...
### Phase 2.8 — ユーザーラベル / タグ（分類・絞り込み）※予定・未着手
- [ ] 動画 / クリップに**ユーザー定義のタグ**（地名・時間帯・被写体・天候など）を複数付与。
  既存の `segment.label`（単一見出し）とは別の複数タグ。任意でカテゴリ（`地名:` `時間帯:` 等）。
- [ ] クリップビュー / ライブラリでタグによる絞り込み・検索。タグ入力は補完付き。
- [ ] 時間帯は `videos.recorded_at` から自動サジェスト（手動上書き可）。
- [ ] 永続化（新テーブル `tags` + 中間表 `item_tags` 等）。
