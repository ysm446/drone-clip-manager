...
- [ ] **再生ボタンを上部プレイヤーへ一本化する**（2026-08-15 起票）: 音楽ビューのツールバーと
  上部プレイヤーに再生ボタンが 2 つ並んでいて紛らわしい。上部だけにしたい。
  - 済: 上部の再生ボタンで **BGM も一緒に止まる / 再開する**（`togglePlay`）。
  - 残: ①音楽ビューが「指定した尺の in/out + BGM」を**常時 App へ装填**する
    （いまはツールバーの再生ボタンを押した時だけ渡している）→ ②音楽ビューのボタンを削除。
  - **注意**: ①は再生中にキューが差し替わると位置が飛ぶ。装填条件（再生中は行わない /
    キューが同一なら差し替えない）を先に固めること。この経路は 2026-08-15 に 2 度壊している
    （停止後に鳴り出す / 先頭に戻る）。
