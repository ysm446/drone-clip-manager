...
- [ ] **クリップの削除**（2026-08-14 起票）: 音楽ビューは追加（パレットからドロップ）と並べ替えは
  できるのに**削除だけできない**。今はノードビューへ切り替えて Delete する必要がある。
  実装自体は既存の `removeSequenceNode` を呼ぶだけで、順路の張り直しも Undo も既存の仕組みに乗る。
  - 操作案: ①クリップを選択して Delete キー（ノードビューと同じ操作で覚えることが増えない）
    ②右クリックメニュー（パレットのクリップカードと同じ流儀）。①を軸に②も併設が素直。
  - 前提: 今の音楽ビューには**選択の概念が無い**ので、先に選択状態を足す必要がある。
    選択の見た目は style-guide の既存ルール（`--selected` / アクセント枠）で表現できる。
