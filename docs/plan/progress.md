...
### シーケンス編集の残作業
- [ ] **拍子（beatsPerBar）を可変にする**（2026-08-15 起票）: いまは `beats.ts` で 4 固定。
  3 拍子の曲では**拍そのものは正しいが小節線がずれ**、「N 小節」の指定が実質「4N 拍」になる。
  破綻はしないが小節という単位が信用できなくなる。
  - `beatsPerBar` は型・DB・描画・吸着をすでに通っているので、**固定値を可変にするだけ**で済む。
    ヘッダに拍子セレクタ（3/4・4/4・6/8）を足し、曲ごとに保存する。
  - 自動判定は「拍子候補ごとに位相別のオンセット強度のコントラストを測る」方法で実装できる
    （`Green From Above 2.wav` で実測: 3 拍子=1.034 とほぼ平坦、4 拍子=1.359 で 3 拍目だけ弱い）。
    ただしこの曲はユーザーの耳では 3 拍子に聞こえるとのことで、**計測と食い違う例がある**。
    自動判定は初期値に留め、手動で上書きできる形にすること（BPM の手動補正と同じ考え方）。
  - 拍子が信用できないため、メトロノームは当面**強弱なし**にしてある。可変にしたら見直す。
